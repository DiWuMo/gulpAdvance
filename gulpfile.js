...
gulp.task('webserver', function() {
  gulp.src('www')
    .pipe(webserver({
      ...
      //实现我们的mock数据
      //原理：1-->用户在浏览器里输入url地址，比如说：http://localhost/queryList
      //2-->系统通过判断，获取到url的地址参数，即queryList
      //3-->通过url的地址参数queryList，去查找相应的json文件，比如说queryList.json
      ...
      middleware:function(req,res,next){
      	...
      	switch(urlObj.pathname){
      		...
      		case '/api/like':
      		res.setHeader('Content-Type','application/json');
      		fs.readFile('mock/like.json','utf-8',function(err,data){
      			res.end(data);
      		});
      		return;
      		case '/api/mine':
